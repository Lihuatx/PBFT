...
def run_commands(arg):
    ...
    for index, (exe, arg1, arg2) in enumerate(filtered_commands):
        ...
        if arg1 in errorNonCommitteeNodeNumList or arg1 in errorCommitteeNodeNumList:
            tmux_command = f"tmux send-keys -t myPBFT:{index + 1} '{exe} {arg1} {arg2} {nodes_per_group} {z} Y' C-m"
        else:
            tmux_command = f"tmux send-keys -t myPBFT:{index + 1} '{exe} {arg1} {arg2} {nodes_per_group} {z}' C-m"
